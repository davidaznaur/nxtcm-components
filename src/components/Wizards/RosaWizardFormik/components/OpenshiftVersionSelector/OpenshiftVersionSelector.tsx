...
import { isSupportedMinorVersion, versionComparator } from '../../helpers/helpers';
import { Version } from './types';
import { getVersionsData } from './helpers';
import { channelGroups, MIN_MANAGED_POLICY_VERSION } from './constants';
...
hasUnstableVersionsCapability: boolean;
unstableClusterVersionFeatureGate?: boolean;
statusVersions: {
    name: string;
    type: string;
}[]
...
function VersionSelection({
  ...
  getInstallableVersionsResponse,
  unstableClusterVersionFeatureGate,
  statusVersions
  ...
  const unstableOCPVersionsEnabled =
    unstableClusterVersionFeatureGate && hasUnstableVersionsCapability;
  ...
  const isValidRosaVersion = React.useCallback(
    ...
    [rosaMaxOSVersion], // rosaMaxOSVersion is actually the max version allowed by the chosen AccountRoles,
  );
  ...
  const isHostedDisabled = React.useCallback(
    (version: Version) => !version.hosted_control_plane_enabled,
    [],
  );
...
//   }, []);
  ...
  useEffect(() => {
    if (versions.length && !selectedClusterVersion?.id) {
      ...
      if (!defaultRosaVersion || !defaultHypershiftVersion) {
  ...
  const { versionsData, hasIncompatibleVersions } = React.useMemo(() => {
    ...
    >((groups, [groupName, groupVersions]) => {
      const processedVersions = (groupVersions as any).map((version: FuzzyEntryType) => {
    ...
    <Switch
      ...
      label={
        <>
          ...
          <Popover
            bodyContent='View only versions that are compatible with a Hosted control plane'
            enableFlip={false}
          >
            <Button
